...
Mooncake 项目的优化问题路由器。根据用户描述的优化问题关键词，匹配到对应的 Mooncake 组件和子组件。

**在开始任何组件级分析之前，必须先阅读 `architecture.md`**——它从三个底层视角（分布式存储、高性能通信、LLM 推理 Serving）剖析 Mooncake 的系统本质。所有组件级优化分析都应参考 architecture.md 中对应的理论框架。

## ⚠️ 前置操作：同步 Mooncake 源码（必须最先执行）

**在分析源代码之前，必须同步 Mooncake 仓库到最新版本：**

```bash
cd <MOONCAKE_REPO 路径> && git pull --rebase
```

- 如果 pull 成功 → 继续，repo-map.md 基于最新代码
- 如果 pull 因网络问题失败 → 告知用户"⚠️ 无法同步 Mooncake 源码，分析基于本地版本"
- 如果有冲突或本地修改 → 先 `git stash`，pull 后再 `git stash pop`，冲突交由用户处理

**更新完源码后，检查 repo-map.md 是否需要更新**：
- 对比 `git log --oneline -5` 的最近变更，查看是否涉及 repo-map.md 中列出的文件
- 如有新增/删除/重命名的关键文件 → 先更新 `repo-map.md` 再继续分析
- 在生成的优化方案中标注所基于的 Mooncake commit hash

## ⚠️ 前置检查：目标仓库路径

按以下优先级确定 Mooncake 源码路径：
...
**`architecture.md`** — Mooncake 三视角统一模型。从分布式存储系统、高性能通信系统、LLM 推理 Serving 三个视角理解 Mooncake 的本质，再深入具体组件。约 300 行，涵盖 CAP 分析、RDMA 性能模型、KV cache 语义等核心理论。
