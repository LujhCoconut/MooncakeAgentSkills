...
- **`update-qa`** — 文本整理入库：长文本 → 提取 Q&A → 源码验证（调用链 ±1 层 + 对抗反驳）→ **预览确认** → 追加到 `qa/KNOWLEDGE.md`；`update-qa audit` 模式重验已有条目防腐（entry rot）
...
- `update-qa/SKILL.md` — 文本整理入库 Q&A（提取 + 源码验证 + 对抗反驳 + 确认后写入）+ audit 防腐审计
