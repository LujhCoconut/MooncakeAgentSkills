...
description: Mooncake 优化分析 · 代码审查 · 快速问答 · 功能设计。子命令 optimize（源码级优化+论文洞察+方案生成）、plan-feature（新功能设计+基础设施复用+方案生成）、code-review（GitHub PR 多 agent 审查）、review（本地 git diff 多维度审查）、qa（概念/配置/故障快速问答）、update-qa（文本整理为 Q&A + 源码验证后入库）、clear-proposals（按 today/month/year 清理历史方案）。覆盖 Transfer Engine、Store、Conductor、框架集成、构建部署、运维监控。
argument-hint: "[optimize|plan-feature|code-review|review|qa|update-qa|clear-proposals] <参数>"
---
...
**子命令分发逻辑**：
1. 如果 args 以 `optimize`、`plan-feature`、`qa`、`update-qa`、`code-review`、`review` 或 `clear-proposals` 开头 → 提取子命令，剩余部分为参数（注意：`update-qa` 的匹配优先于 `qa`，先检查长前缀）
2. 如果无显式子命令 → 判断参数特征：
