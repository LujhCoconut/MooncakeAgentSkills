...
description: Mooncake 优化分析 · 代码审查 · 快速问答 · 功能设计 · API 参考 · 故障排查 · CI 验证。子命令 optimize（源码级优化+论文洞察+方案生成）、plan-feature（新功能设计+基础设施复用+方案生成）、code-review（GitHub PR 多 agent 审查）、review（本地 git diff 多维度审查）、qa（概念/配置/故障快速问答）、update-qa（文本整理为 Q&A + 源码验证后入库）、clear-proposals（按 today/month/year 清理历史方案）、api-reference（Python API 参考指南+源码映射）、troubleshoot（系统化部署故障诊断）、ci-local（提交前本地 CI 验证）。覆盖 Transfer Engine、Store、Conductor、框架集成、构建部署、运维监控。
argument-hint: "[optimize|plan-feature|code-review|review|qa|update-qa|clear-proposals|api-reference|troubleshoot|ci-local] <参数>"
---
...
- **`review`** — 本地代码审查：`git diff` 多维度分析 (comments/tests/errors/types/code/simplify/perf-claims) → 结构化报告
- **`qa`** — 快速问答：概念解释、配置查询、故障排查（检索 `qa/KNOWLEDGE.md`）
- **`update-qa`** — 文本整理入库：长文本 → 提取 Q&A → 源码验证（调用链 ±1 层 + 对抗反驳）→ **预览确认** → 追加到 `qa/KNOWLEDGE.md`；`update-qa audit` 模式条目重验（对照当前源码，防 entry rot）
- **`clear-proposals`** — 清理历史方案：按 today/month/year 删除 proposals，交互确认后执行
- **`api-reference`** — Python API 参考指南：Store / Transfer Engine / EP Backend 完整 API 用法、代码示例、API→源码映射、最佳实践（适配自 kvcache-ai/Mooncake 官方 skill）
- **`troubleshoot`** — 系统化部署故障诊断：8 步检查 (服务/RDMA/环境变量/网络/内存/日志/配置) → 结构化报告（含严重度分级）→ 关联 QA 入口（适配自 kvcache-ai/Mooncake 官方 skill）
- **`ci-local`** — 提交前本地 CI 验证：运行 `scripts/run_ci_test.sh` + 变更分析 + quick 快速模式 + 失败根因定位（适配自 kvcache-ai/Mooncake 官方 skill）
...
| `update-qa` | 文本整理入库 Q&A | `/mooncake-agent-skills update-qa "<一大段文本>"` |
| `clear-proposals` | 清理历史方案 | `/mooncake-agent-skills clear-proposals today` |
| `api-reference` | Python API 参考指南 | `/mooncake-agent-skills api-reference "Store put_tensor 怎么用"` |
| `troubleshoot` | 部署故障诊断 | `/mooncake-agent-skills troubleshoot` 或 `/mooncake-agent-skills troubleshoot "RDMA 连接失败"` |
| `ci-local` | 提交前本地 CI | `/mooncake-agent-skills ci-local` 或 `/mooncake-agent-skills ci-local --quick` |
```
...
| `review` | 展示维度选项：`[all]`（默认）\| `comments` \| `tests` \| `errors` \| `types` \| `code` \| `simplify` \| `perf-claims`，提示「请选择审查维度，默认 all」 |
| `qa` | 提示「请输入你的问题」 |
| `update-qa` | 展示模式选项：「粘贴文本入库」（默认）\| 「audit 审计已有条目」 |
| `clear-proposals` | 展示时间范围选项：`today` \| `month` \| `year`，提示「请选择时间范围」 |
| `api-reference` | 展示组件选项：`store` \| `engine` \| `ep` \| `env` \| `pattern`，提示「请选择 API 组件，或直接描述需求」 |
| `troubleshoot` | 提示「请描述故障现象或粘贴错误信息」，若无则执行 quick 快速诊断 |
| `ci-local` | 展示模式选项：「完整 CI」\| `--quick` 快速门禁 \| `--skip-path-filter` 强制全量，提示「请选择 CI 模式，默认完整 CI」 |
...
| 以 `update-qa` 开头，或要求「把这段整理进 QA」「加到问答库」「入库」等 | `update-qa` | `/mooncake-agent-skills update-qa "<一大段文本>"` |
...
| 以 `api-reference` 开头，或问 API 用法/代码示例 | `api-reference` | `/mooncake-agent-skills api-reference "Store put_tensor 怎么用"` 或 `/mooncake-agent-skills "MooncakeDistributedStore 的 setup 参数"` |
| 以 `troubleshoot` 开头，或要求排查/诊断/修复故障 | `troubleshoot` | `/mooncake-agent-skills troubleshoot` 或 `/mooncake-agent-skills troubleshoot "RDMA QP 创建失败"` |
| 以 `ci-local` 开头，或要求跑本地 CI/提交前验证 | `ci-local` | `/mooncake-agent-skills ci-local` 或 `/mooncake-agent-skills ci-local --quick` |

**子命令分发逻辑**：
1. 如果 args 以 `optimize`、`plan-feature`、`qa`、`update-qa`、`code-review`、`review`、`clear-proposals`、`api-reference`、`troubleshoot` 或 `ci-local` 开头 → 提取子命令，剩余部分为参数（注意：`update-qa` 的匹配优先于 `qa`，`api-reference` 优先于 `optimize` 和 `qa`，先检查长前缀）
2. 如果无显式子命令 → 判断参数特征：
   ...
   - 包含「API」「怎么用」「代码示例」「import」「方法签名」「参数列表」等 → 走 `api-reference` 流程
   - 包含「排查」「诊断」「修复」「报错」「启动失败」「连接不上」「troubleshoot」「debug」等 → 走 `troubleshoot` 流程
   - 包含「CI」「本地验证」「提交前」「pre-submit」「run_ci_test」等 → 走 `ci-local` 流程
   - 包含「是什么」「怎么」「为什么」「报错」「配置」「默认值」「怎么调」等 → 走 `qa` 流程
   - 包含「整理进 QA」「加到问答库」「入库」「补充 Q&A」等 → 走 `update-qa` 流程
...
5. `update-qa` 子命令：读取 `update-qa/SKILL.md` 执行文本整理 + 源码验证 + 入库流程
6. `clear-proposals` 子命令：读取 `clear-proposals/SKILL.md` 按时间范围清理历史方案
7. `plan-feature` 子命令：读取 `plan-feature/SKILL.md` 执行新功能设计分析流程
8. `api-reference` 子命令：读取 `api-reference/SKILL.md` 提供 Mooncake Python API 参考指南
9. `troubleshoot` 子命令：读取 `troubleshoot/SKILL.md` 执行系统化部署故障诊断
10. `ci-local` 子命令：读取 `ci-local/SKILL.md` 执行提交前本地 CI 验证
...
11. 评估每条洞察的可应用性：
    ...
    - **不适用**：明确说明原因
12. 检查 `proposals/optimize/` 目录是否已有相关方案，避免重复；**同时检查 `history/rejected-proposals.md`（whiteboard）**——被否决过的方案不得原样重提，除非能针对否决理由给出新证据（此时预览中须明示「曾被否决：<理由>；本次不同点：<说明>」）
...
**上下文分离（推荐）**：方案起草（洞察评估 + 建议修改的形成）建议交给**独立 agent** 执行，输入为 Phase 2/4.5 的结构化摘要（组件、关键函数清单、✅/⚠️/❌ gap 标注、论文洞察要点），而非携带全部源码阅读上下文——单一上下文被已读代码淹没后，倾向于提出局部小改而非结构性方案（Jitskit(arXiv'26) 的 Planner/Coder 分离：planner 独立上下文使其能提出结构性重设计，ablation 显示 +10.3% 收敛收益）。
...
22. **仅在用户选择「生成完整方案」后才写入方案文件**。如果选择「调整方案范围」，根据用户补充说明修改预览后重新确认。如果选择「放弃」，不生成文件，但**必须**将方案标题与否决理由追加到 `history/rejected-proposals.md`（whiteboard），避免后续会话重复提出——用户未说明理由时询问一句；仍不说明则记「用户未说明」+ 预览要点。

### Phase 6: 输出方案

23. 按方案模板生成优化方案到 `proposals/optimize/<project>-<component>-<slug>-<date>.md`
24. 更新 `history/optimization-log.md`
25. 向用户呈现方案摘要，末尾附**上下文消耗统计**：

```
> **上下文消耗**: 本次分析读取 A 个文件 / B 行源码 / 约 C tokens。检索领域知识 D 篇论文 / E 个 KNOWLEDGE.md。借鉴 code-review-graph 的 context_savings 自汇报机制。
```

> ⚠️ **方案内容不回写 KNOWLEDGE.md**：optimize 产出只写入 `proposals/` 和 `history/optimization-log.md`。组件 `KNOWLEDGE.md` 由人工维护，禁止自动追加方案洞察。
...
15. 更新 `history/optimization-log.md`（标注为 `plan-feature`）
16. 向用户呈现方案摘要，末尾附**上下文消耗统计**（格式同 optimize Phase 6 第 25 步）

> ⚠️ **方案内容不回写 KNOWLEDGE.md**：plan-feature 产出只写入 `proposals/` 和 `history/optimization-log.md`。组件 `KNOWLEDGE.md` 由人工维护，禁止自动追加方案洞察。
...
> **预计影响范围**: N 个文件 · M 个子组件 · K 个下游调用点 · 回归风险: Low/Medium/High（依据：改动波及的 caller 数量 + 跨组件边界 + 是否涉及接口/协议变更。借鉴 code-review-graph 的 blast radius 概念——改动越靠近调用图核心节点，影响半径越大）

## 预期收益
<量化预估，含置信度。**每项收益必须给出对应的 leading indicator**——除了端到端指标（延迟/吞吐），说明通过哪个中间信号观测收益的形成（锁竞争、cache 命中率、I/O 计数、内存带宽、对应 queueing-theory 排队点的 ρ/等待延迟）。只有端到端指标无法诊断"为什么"（Jitskit(arXiv'26) ablation：有无 leading indicators 的收敛差异达 1.42-3.75×）>
- **延迟**: ... （leading indicator: ...）
- **吞吐**: ... （leading indicator: ...）
- **资源利用率**: ...
- **可靠性**: ...
...
<验证设计必须防 reward-hack（借鉴 Jitskit(arXiv'26) 的四条作弊轴）：>
- **避免 derivable ground truth**：验证数据含不可重构熵（per-run 随机 tag / 隐藏种子），防止"重算代替存储"型假收益
- **per-run 随机化 key 空间**：防止优化利用测试数据的密度/顺序规律；若密集 key 等属性是真实部署属性，应显式写入工作负载假设而非随机化掉
- **状态化验证**：加入重启恢复、内存压力等场景，强制数据真实物化，仅输出比对不够
- **装置对等**：优化前后/对照组的 benchmark 配置逐项相同（value 大小、batch、预热状态），采样避开 warmup 窗口
- **归因验证**：leading indicators 与端到端指标同时采集，收益须能归因到方案声称的机制；无法归因的收益按可疑处理
```

## 输出规范

- **引用来源**：方案开头说明参考了哪些 SKILL.md、KNOWLEDGE.md 和论文
- **方案不回写 KNOWLEDGE.md**：optimize / plan-feature 的方案内容只写入 `proposals/` 和 `history/optimization-log.md`；组件 `KNOWLEDGE.md` 由人工维护，本 skill 只读不写
...
- `review/SKILL.md` — 本地代码审查（git diff 多维度分析，含 perf-claims 收益真实性维度）
- `clear-proposals/SKILL.md` — 清理历史方案（按 today/month/year）
- `plan-feature/SKILL.md` — 新功能设计方案（基础设施复用 + 知识库设计模式）
- `qa/SKILL.md` — Mooncake 快速问答
- `update-qa/SKILL.md` — 文本整理入库 Q&A（提取 + 源码验证 + 对抗反驳 + 确认后写入）+ audit 条目重验
- `api-reference/SKILL.md` — Mooncake Python API 参考指南（Store / Transfer Engine / EP Backend + API→源码映射 + 最佳实践）
- `troubleshoot/SKILL.md` — Mooncake 部署故障系统化诊断（8 步检查 + 严重度分级 + 错误码→源码映射 + QA 关联）
- `ci-local/SKILL.md` — Mooncake 提交前本地 CI 验证（变更分析 + quick 模式 + 失败根因定位）
...
**在完成所有变更（新建方案、追加优化记录等）之后，执行以下两步流程：**
...
**规则**：
- `<变更摘要>` 格式：`"proposals(optimize): <方案标题>"` 或 `"proposals(feature): <方案标题>"` 或 `"knowledge(qa): add <N> Q&A entries via update-qa"` 或 `"knowledge: update <组件> optimization targets"` 或 `"skill: <变更描述>"`
