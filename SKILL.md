...
description: Mooncake 优化分析 · 代码审查 · 快速问答。子命令 optimize（源码级优化+论文洞察+方案生成）、code-review（GitHub PR 多 agent 审查）、review（本地 git diff 多维度审查）、qa（概念/配置/故障快速问答）。覆盖 Transfer Engine、Store、Conductor、框架集成、构建部署、运维监控。
---
...
**子命令分发逻辑**：
1. 如果 args 以 `optimize`、`qa`、`code-review` 或 `review` 开头 → 提取子命令，剩余部分为参数
2. 如果无显式子命令 → 判断参数特征：
