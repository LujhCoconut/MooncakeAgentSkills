...
/mooncake-agent-skills review quick         # 快速门禁（Overall Risk + Top 5 + Missing Tests）
...
| **Local Review** | `review/` | 1 | git diff 多维度分析 (comments/tests/errors/types/code/simplify/perf-claims/quick) + Overall Risk + Missing Tests + context savings（依赖 Claude Code） |
...
│
├── review/                      # 本地代码审查
│   └── SKILL.md                 #   git diff 多维度 (comments/tests/errors/types/code/simplify/perf-claims/quick) + Overall Risk + Missing Tests + context savings（依赖 Claude Code）
│
...
| 条目重验 | `update-qa audit` 模式 | Auditor：定期审读实现 vs 规范，失效点显式化 |
...
| Overall Risk + Blast Radius + Missing Tests | review 聚合输出 | impact radius（>20 nodes → high）+ test gap 显式清单 |
| quick 模式（minimal 输出） | `review quick` | `detail_level="minimal"`，~100 tokens 摘要 |
| Context savings 自汇报 | review/code-review/optimize 输出末尾 | `attach_context_savings()` — 文件数/行数/tokens/节省比例 |
| 方案影响范围 | 方案模板「建议修改」后 | blast radius：波及文件数/子组件/下游调用点/回归风险 |
| 静态映射 → 实时图补充 | `mooncake/repo-map.md` 注释 | Leiden 聚类 + auto-naming + architecture overview |
