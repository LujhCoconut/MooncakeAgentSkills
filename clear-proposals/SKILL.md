...
按时间范围删除 `proposals/optimize/` 和 `proposals/feature/` 目录下的方案文件，以 git log 中该文件的**最近一次 commit 时间**为依据。
...
如果用户未指定参数或参数不在 `today/month/year` 中，使用 `AskUserQuestion` 展示选项让用户选择：`today` | `month` | `year`（附带 "Other" 选项供手动输入）。
...
```bash
# 获取所有 proposal 文件列表
ls proposals/optimize/*.md proposals/feature/*.md 2>/dev/null

# 对每个文件，检查其最近一次 commit 时间是否在范围内
for f in proposals/optimize/*.md proposals/feature/*.md; do
  [ -f "$f" ] || continue
  last_commit=$(git -C "$SKILL_DIR" log -1 --format="%ai" -- "$f" 2>/dev/null)
...
|------|-----------------|---------|
| proposals/optimize/mooncake-store-bucket-ssd-offloading-2026-07-16.md | 2026-07-16 15:30:00 | 12.3 KB |
| proposals/feature/mooncake-store-cloud-storage-integration-2026-07-16.md | 2026-07-16 16:02:00 | 15.7 KB |
...
```bash
cd <本 skill 所在目录>
git rm proposals/optimize/<file1>.md proposals/feature/<file2>.md ...
git commit -m "housekeeping: clear proposals <today|month|year> (<N_optimize> + <N_feature> files)" && git push
```

**规则**：
- 如果 `git diff --cached` 为空（无文件匹配） → 告知用户「该时间范围内无方案文件」，不 commit
- commit 信息格式：`"housekeeping: clear proposals <today|month|year> (<N_optimize> + <N_feature> files)"`
...
| 场景 | 行为 |
|------|------|
| `proposals/optimize/` 和 `proposals/feature/` 均为空 | 提示「无方案文件可清理」 |
| 时间范围内无匹配 | 提示「未找到 <范围> 内的方案文件」 |
| 文件已通过其他方式删除（git 中已 staged 为 deleted） | 跳过，不重复删除 |
| `proposals/` 目录不存在 | 提示后退出 |
| 仅 optimize 或仅 feature 有匹配 | 只删除有匹配的那个子目录文件，commit 信息如实标注 |
